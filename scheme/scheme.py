...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(operate,MacroProcedure):
            return scheme_eval(operate.apply_macro(rest,env),env)
        operands = rest.map(lambda obj: scheme_eval(obj,env))
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        if type(expressions.rest.first) is int or type(expressions.rest.first) is float:
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        ...
        done_macro_form = MacroProcedure(formals,body,env)
        env.define(proc_name, done_macro_form)
        return proc_name
    raise SchemeError('Incorrect Form')
    # END Problem 20
